...
description: >
  Review the current diff against capybaraa's rules: lean, ask, optimal, terse, clean,
  sync. Finds guessed specs, speculative abstractions, bad complexity, filler comments,
  mess left behind, and docs/tests left out of sync. One line per finding, lists only,
  does not apply fixes.
  Use when the user says "review against the rules", "capybaraa review", "/capybaraa-review",
...
Detailed guidance: [`references/principles.md`](../../references/principles.md).
...
- `terse:` filler comments that restate the code, dead prose, a comment that explains
  the obvious. Name the lines to cut.
- `clean:` the old version left next to the new, a stale comment, dead code the change
...
Spotted a dropped guard (validation, error handling, security)? Flag it in one line too.
